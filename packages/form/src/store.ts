...
export class SimpleForm<T = unknown> {
  ...
  bindChange(path?: FormPathType, eventName?: string, ...args) {
    ...
    const newValue =
      typeof valueGetter == "function" ? valueGetter(...args) : undefined;
    this.setFieldValue(path, newValue, eventName);
    // 主动onchange事件
    onFieldsChange &&
      onFieldsChange({ name: path, value: newValue }, this.getFieldValue());
  }

  // 给目标控件绑定的props
  getBindProps(path?: FormPathType, newValue?) {
    if (!isValidFormName(path)) return;
    const props = this.getFieldProps(path);
    const currentValue =
      newValue !== undefined ? newValue : this.getFieldValue(path);
    const { valueProp, valueSetter, trigger, validateTrigger, rules, nonform } =
      props || {};
    const valuePropName = getValuePropName(valueProp) || "";
    const triggers = mergeTriggers(
      trigger,
      validateTrigger,
      getRulesTriggers(rules)
    );
    const childValue =
  ...
  public getInitialValues(path?: FormPathType) {
    return path === undefined
      ? this.initialValues
      : deepGet(this.initialValues, path);
  }

  // 更新表单值，单个表单值或多个表单值
  public setFieldValue(
    path: object,
    eventName?: Parameters<typeof isCanTrigger>[0]
  ): void;
  public setFieldValue(
    path: FormPathType,
    value?: unknown,
  ...
  public setFieldValue(...args) {
    ...
    if (isObject(args?.[0]) || isArray(args?.[0])) {
      Promise.all(
        Object.keys(args[0]).map((n) =>
          setFormItemValue(n, args[0]?.[n], args[1])
        )
      );
    } else {
  ...
  public getFieldError(path: FormPathType | undefined): GetMapValueType<FormErrors>;
  ...
  private setFieldError(path: FormPathType, val?: GetMapValueType<FormErrors>) {
    if (!isValidFormName(path)) return;
    if (this.getFieldError(path) === val) return;
    if (val === undefined) {
  ...
  public async validate(
    path?: FormPathType | Array<FormPathType>,
    eventName?: Parameters<typeof isCanTrigger>[0]
  ) {
    ...
    if (!(path instanceof Array) && path !== undefined) {
      ...
      const keys =
        path instanceof Array ? path : Array.from(fieldPropsMap.keys());
  ...
  public subscribeFormItem(
    path?: FormPathType,
    listener?: SimpleForm<T>["formItemListeners"][number]["onChange"]
  ) {
  ...
  public unsubscribeFormItem(path?: FormPathType) {
    if (path === undefined) {
      ...
      this.formItemListeners = this.formItemListeners.filter(
        (sub) => !isEqualName(sub.path, path)
      );
    }
  }
  // 同步当前表单域值的变化
  private notifyFormItem(path?: FormPathType) {
    if (path === undefined) {
      this.formItemListeners.forEach((listener) =>
        listener.onChange(
          this.getFieldValue(listener.path),
          this.getLastValue(listener.path)
        )
      );
    } else {
      this.formItemListeners.forEach((listener) => {
  ...
  public subscribeFormValue(
    listener: FormListener<unknown>["onChange"]
  ): () => void;
  public subscribeFormValue(
    path: FormPathType,
    listener: FormListener<unknown>["onChange"]
  ...
  public subscribeFormValue(
    path: FormPathType | FormListener<unknown>["onChange"],
    listener?: FormListener<unknown>["onChange"]
  ) {
  ...
  public unsubscribeFormValue(path?: FormPathType) {
    if (path === undefined) {
      ...
      this.formValueListeners = this.formValueListeners.filter(
        (sub) => !isEqualName(sub.path, path)
      );
    }
  }
  ...
  public notifyGlobalForm() {
    this.globalFormListeners.forEach((listener) =>
      listener?.(this.getFieldValue(), this.getLastValue())
    );
  }
  ...
  public subscribeError(
    path?: FormPathType,
    listener?: SimpleForm<T>["errorListeners"][number]["onChange"]
  ) {
    if (!isValidFormName(path) || !listener) return;
  ...
  private notifyError(path?: FormPathType) {
    if (path === undefined) {
      this.errorListeners.forEach((listener) =>
        listener.onChange(this.getFieldError(listener.path))
      );
    } else {
      this.errorListeners.forEach((listener) => {
