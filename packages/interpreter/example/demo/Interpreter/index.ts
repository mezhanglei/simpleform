...
import { NodeState, FactoryInstance, ScopeInstance, TaskInstance } from './typings';
...
function Interpreter(code, opt_initFunc) {
  const ast = typeof code === 'string' ? this.parse_(code, 'code') : code;
  ...
  const cloneNode = cloneObject(this.newNode(), ast);
  /**
  ...
  this.ast = cloneNode;
  const state = new Interpreter.State(this.ast, this.globalScope);
  state.done = false;
...
Interpreter.prototype.parse_ = function (code, sourceFile) {
  const options = cloneObject<acorn.Options>(Interpreter.PARSE_OPTIONS, { sourceFile });
...
Interpreter.prototype.step = function () {
  ...
  do {
    ...
    const oldInterpreterValue = Interpreter.currentInterpreter_;
...
Interpreter.prototype.createTask_ = function (isInterval, args) {
  ...
  if ((exec instanceof Interpreter.Object) && exec.class === 'Function') {
    ...
    stripLocations(node, execStart, execEnd);
...
Interpreter.prototype.initString = function (globalObject) {
  ...
  wrapper = function split(separator, limit, callback) {
    ...
    if (this.isInherit(separator, thisInterpreter.REGEXP)) {
  ...
  wrapper = function match(regexp, callback) {
    var string = String(this);
    regexp = this.isInherit(regexp, thisInterpreter.REGEXP) ?
  ...
  wrapper = function search(regexp, callback) {
    var string = String(this);
    if (this.isInherit(regexp, thisInterpreter.REGEXP)) {
      regexp = regexp.data;
    } else {
  ...
  wrapper = function replace_(substr, newSubstr, callback) {
    ...
    if (this.isInherit(substr, thisInterpreter.REGEXP)) {
      substr = substr.data;
...
Interpreter.prototype.initRegExp = function (globalObject) {
  ...
  wrapper = function RegExp(pattern, flags) {
    if (thisInterpreter.calledWithNew()) {
      ...
      // Called as `RegExp()`.
      if (flags === undefined &&
        this.isInherit(pattern, thisInterpreter.REGEXP)) {
 ...
 *     False otherwise.
 */
Interpreter.prototype.isInherit = function (child, constructor) {
...
Interpreter.prototype.createObjectProto = function (proto) {
  ...
  const obj = new Interpreter.Object(proto);
  if (this.isInherit(obj, this.ERROR)) {
    obj.class = 'Error';
  }
  return obj;
};
...
Interpreter.prototype.pseudoToNative = function (pseudoObj, opt_cycles) {
  ...
  if (this.isInherit(pseudoObj, this.REGEXP)) { // Regular expression.
  ...
  if (this.isInherit(pseudoObj, this.DATE)) { // Date.
  ...
  // Boxed primitives.
  if (this.isInherit(pseudoObj, this.NUMBER) ||
    this.isInherit(pseudoObj, this.STRING) ||
    this.isInherit(pseudoObj, this.BOOLEAN)) {
  ...
  var nativeObj = this.isInherit(pseudoObj, this.ARRAY) ? [] : {};
  cycles.native.push(nativeObj);
  var val;
...
Interpreter.prototype.getProperty = function (obj, name) {
  ...
  if (name === 'length') {
    ...
    if (this.isInherit(obj, this.STRING)) {
...
Interpreter.prototype.hasProperty = function (obj, name) {
  ...
  name = String(name);
  if (name === 'length' && this.isInherit(obj, this.STRING)) {
    return true;
  }
  if (this.isInherit(obj, this.STRING)) {
    var n = Interpreter.legalArrayIndex(name);
...
Interpreter.prototype.unwind = function (type, value, label) {
  ...
  var realError;
  if (this.isInherit(value, this.ERROR)) {
    var errorTable = {
      'EvalError': EvalError,
  ...
  if (!state.doneExec_) {
    ...
    if (funcNode) {
      ...
      if (typeof code !== 'string') {
        ...
        stripLocations(evalNode, node.start, node.end);
