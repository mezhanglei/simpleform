...
import { getStackError } from "./global/Error";
import { isArray, isString } from "./global/type";
import { AcornNode } from "./typings";
import { bindClassPrototype, getPropInPrototypeChain, legalArrayIndex, legalArrayLength } from "./utils";
...
class ObjectConstructor {
  ...
  throwException(errorName: string, opt_message, script) {
    const name = String(errorName);
    const message = opt_message?.valueOf();
  ...
  setProperty(name, value, opt_descriptor, script) {
    ...
    if (isString(obj)) {
      ...
      if (name === 'length' || (!isNaN(n) && n < String(obj).length)) {
        ...
        if (strict) {
          this.throwException('TypeError', "Cannot assign to read only " +
    ...
    if (isArray(obj)) {
      ...
      if (name === 'length') {
        ...
        if (isNaN(maxLen)) {
          this.throwException('RangeError', 'Invalid array length', script);
        }
        if (maxLen < len) {
    ...
    if (obj.preventExtensions && !(name in obj.properties)) {
      if (strict) {
        this.throwException('TypeError', "Can't add property '" + name +
    ...
    if (opt_descriptor) {
      if (opt_descriptor && ('get' in opt_descriptor || 'set' in opt_descriptor) &&
        ...
        this.throwException('TypeError', 'Invalid property descriptor. ' +
      ...
      try {
        ...
        this.throwException('TypeError', 'Cannot redefine property: ' + name, script);
      }
      ...
      if (defObj.getter && defObj.getter[name]) {
        if (strict) {
          this.throwException('TypeError', "Cannot set property '" + name +
