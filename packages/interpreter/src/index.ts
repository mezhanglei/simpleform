...
import { AstNodeExecution } from './execution';
...
const Interpreter = function (...args) {
  ...
  if (typeof args?.[0] === 'string' || args?.[0]?.type === 'Program') {
    // 默认js-interpreter
    this.initGlobal(args[1]);
    this.initCode(args[0]);
  } else {
    // 对象入参
    this.initGlobal(args[0]?.initFunc);
    this.initCode(args[0]?.code);
  }
};

Interpreter.Completion = Constants.Completion;
Interpreter.Status = Constants.Status;
Interpreter.PARSE_OPTIONS = Constants.PARSE_OPTIONS;
Interpreter.READONLY_DESCRIPTOR = Constants.READONLY_DESCRIPTOR;
Interpreter.NONENUMERABLE_DESCRIPTOR = Constants.NONENUMERABLE_DESCRIPTOR;
Interpreter.READONLY_NONENUMERABLE_DESCRIPTOR = Constants.READONLY_NONENUMERABLE_DESCRIPTOR;
Interpreter.NONCONFIGURABLE_READONLY_NONENUMERABLE_DESCRIPTOR = Constants.NONCONFIGURABLE_READONLY_NONENUMERABLE_DESCRIPTOR;
Interpreter.VARIABLE_DESCRIPTOR = Constants.VARIABLE_DESCRIPTOR;
Interpreter.STEP_ERROR = Constants.STEP_ERROR;
Interpreter.SCOPE_REFERENCE = Constants.SCOPE_REFERENCE;
Interpreter.VALUE_IN_DESCRIPTOR = Constants.VALUE_IN_DESCRIPTOR;
Interpreter.REGEXP_TIMEOUT = Constants.REGEXP_TIMEOUT;
Interpreter.vm = Constants.vm;
Interpreter.nativeGlobal = Object.assign((typeof globalThis === 'undefined') ? this || window : globalThis, { acorn });
Interpreter.WORKER_CODE = Constants.WORKER_CODE;
Interpreter.Context = Context;
Interpreter.Execution = AstNodeExecution;
Interpreter.Object = ObjectConstructor;
Interpreter.State = StateConstructor;
Interpreter.Scope = ScopeConstructor;
Interpreter.Task = TaskConstructor;
...
Interpreter.prototype.parse_ = function (code, sourceFile) {
  ...
  return typeof code === 'string' ? Interpreter.nativeGlobal.acorn.parse(code, options) : cloneASTNode(code);
};

/**
 * create new node
 */
Interpreter.prototype.newNode = function () {
  const ast = Interpreter.nativeGlobal.acorn.parse('');
...
Interpreter.prototype.initPolyfill = function () {
  const ast = this.parse_(polyfills.join('\n'), 'polyfills');
  stripLocations_(ast, undefined, undefined);
  const state = new Interpreter.State(ast, this.globalScope);
  state.done = false;
...
Interpreter.prototype.initCode = function (code) {
  ...
  const ast = this.parse_(code, 'code');
  this.ast = ast;
  this.globalScope.isStrict = isStrict(ast);
  this.populateScope_(ast, this.globalScope);
  const state = new Interpreter.State(ast, this.globalScope);
  state.done = false;
  // 执行栈
  this.stateStack = state.node ? [state] : [];
 ...
 * 设置上下文环境
 */
Interpreter.prototype.initGlobal = function (initFunc) {
...
Interpreter.prototype.initObject = function (globalObject) {
  ...
  wrapper = function Object(value) {
    ...
    if (!(value instanceof Interpreter.Object)) {
      ...
      var box = new Interpreter.Object(
  ...
  wrapper = function getOwnPropertyDescriptor(obj, prop) {
    ...
    var pseudoDescriptor =
      new Interpreter.Object(thisInterpreter.OBJECT_PROTO);
    if (getter || setter) {
 ...
 */
Interpreter.prototype.createScope = function (node, parentScope = null) {
...
Interpreter.prototype.populateScope_ = function (node, scope) {
  ...
  const variableCache = traverseAstDeclar(node, (currentCache) => {
    for (const name in currentCache) {
      // 给作用域填充变量或者函数
      const val = currentCache[name] === true ? undefined : this.createFunction(currentCache[name], scope);
      this.setProperty(scope.object, name, val, Interpreter.VARIABLE_DESCRIPTOR);
    }
  });
...
Interpreter.prototype.throwException = function (errorClass, opt_message) {
  ...
  if (opt_message === undefined ||
    ...
    var error = new Interpreter.Object(errorClass.properties['prototype']);
    error.class = 'Error';
