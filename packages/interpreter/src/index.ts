...
import { InterpreterOtionFun } from './typings';
...
// 作用域构造函数
class Interpreter {
  static Completion = StaticConstants.Completion;
  static Status = StaticConstants.Status;
  static PARSE_OPTIONS = StaticConstants.PARSE_OPTIONS;
  static READONLY_DESCRIPTOR = StaticConstants.READONLY_DESCRIPTOR;
  static NONENUMERABLE_DESCRIPTOR = StaticConstants.NONENUMERABLE_DESCRIPTOR;
  static READONLY_NONENUMERABLE_DESCRIPTOR = StaticConstants.READONLY_NONENUMERABLE_DESCRIPTOR;
  static NONCONFIGURABLE_READONLY_NONENUMERABLE_DESCRIPTOR = StaticConstants.NONCONFIGURABLE_READONLY_NONENUMERABLE_DESCRIPTOR;
  static VARIABLE_DESCRIPTOR = StaticConstants.VARIABLE_DESCRIPTOR;
  static STEP_ERROR = StaticConstants.STEP_ERROR;
  static SCOPE_REFERENCE = StaticConstants.SCOPE_REFERENCE;
  static VALUE_IN_DESCRIPTOR = StaticConstants.VALUE_IN_DESCRIPTOR;
  static REGEXP_TIMEOUT = StaticConstants.REGEXP_TIMEOUT;
  static vm = StaticConstants.vm;
  static WORKER_CODE = StaticConstants.WORKER_CODE;
  static State = StaticConstants.State;
  static Scope = StaticConstants.Scope;
  static Object = StaticConstants.Object;
  static Task = StaticConstants.Task;
  // 原生全局变量
  static nativeGlobal = Object.assign((typeof globalThis === 'undefined') ? this || window : globalThis, { acorn });

  initFunc_?: InterpreterOtionFun;
  stepFunctions_: typeof getStepFunctions;
  paused_: boolean;
  functionCounter_: number;
  tasks: TaskConstructor[];
  ast: acorn.Program;
  globalScope: ScopeConstructor;
  stateStack: Array<StateConstructor>;
  value: unknown;
  // Some pathological regular expressions can take geometric time Regular expressions are handled in one of three ways:
  // 0 - throw as invalid. 1 - execute natively (risk of unresponsive program). 2 - execute in separate thread (not supported by IE 9).
  REGEXP_MODE: number = 2;
  // If REGEXP_MODE = 2, the length of time (in ms) to allow a RegExp thread to execute before terminating it.
  ...
  parse_(code, sourceFile) {
    const options = assignProperty<acorn.Options>({}, Interpreter.PARSE_OPTIONS, { sourceFile });
    return Interpreter.nativeGlobal.acorn.parse(code, options);
  };

  setCode(code, context?: unknown) {
    this.value = undefined;
    this.tasks = [];
    this.paused_ = false;
    this.functionCounter_ = 0;
    this.ast = typeof code === 'string' ? this.parse_(code, 'code') : code;
    const cloneNode = cloneASTNode(this.ast);
    stripLocations_(cloneNode, undefined, undefined);
    this.globalScope = this.createScope(cloneNode, null);
    this.initGlobal(this.globalScope.object);
    const state = new Interpreter.State(cloneNode, this.globalScope);
    state.done = false;
    this.stateStack = state.node ? [state] : [];
  }

  // Add more code to the interpreter.
  appendCode(code, context?: unknown) {
    const state = this.stateStack?.[0];
    if (!state || state.node.type !== 'Program') {
      throw Error('Expecting original AST to start with a Program node');
    }
    const ast = typeof code === 'string' ? this.parse_(code, 'appendCode' + (this.appendCodeNumber_++)) : code;
  ...
  createScope(node, parentScope) {
    ...
    const object = new Interpreter.Object(null);
    const scope = new Interpreter.Scope(parentScope, strict, object);
    this.populateScope_(node, scope);
    return scope;
  };

  initGlobal(globalObject) {
    // 1. uneditable global properties.
    [
      ['NaN', NaN],
      ['Infinity', Infinity],
      ['undefined', undefined],
      ...
      this.setProperty(globalObject, key, val,
        Interpreter.NONCONFIGURABLE_READONLY_NONENUMERABLE_DESCRIPTOR);
    });
    // 2. editable global properties.
    ...
    function decodeWrapper(nativeFunc: Function) {
    ...
    [
      ['eval', function (_x) { throw EvalError("Can't happen"); }],
      ['parseInt', parseInt],
      ['parseFloat', parseFloat],
      ['isNaN', isNaN],
      ['isFinite', isFinite],
      ['escape', decodeWrapper(escape)],
      ['unescape', decodeWrapper(unescape)],
      ['decodeURI', decodeWrapper(decodeURI)],
      ['encodeURI', decodeWrapper(encodeURI)],
      ['decodeURIComponent', decodeWrapper(decodeURIComponent)],
      ['encodeURIComponent', decodeWrapper(encodeURIComponent)],
      ['setTimeout', function setTimeout(...args) {
        return thisInterpreter.createTask_(false, args);
      }],
      ['setInterval', function setInterval(...args) {
        return thisInterpreter.createTask_(true, args);
      }],
      ['clearTimeout', function clearTimeout(pid) {
        thisInterpreter.deleteTask_(pid);
      }],
      ['clearInterval', function clearInterval(pid) {
        thisInterpreter.deleteTask_(pid);
      }]
    ].forEach(([key, wrapper]) => {
      const func = this.createNativeFunction(wrapper, false);
      if (key === 'eval') {
        func.eval = true;
      }
      this.setProperty(globalObject, key, func,
  ...
  step() {
    ...
    do {
      ...
      // 多线程或嵌套调用场景, 赋值当前解释器实例
      const oldInterpreterValue = Interpreter.Object.currentInterpreter_;
      Interpreter.Object.currentInterpreter_ = this;
      let nextState;
      try {
        nextState = this.stepFunctions_[node.type](stack, state);
      } catch (e) {
...
Interpreter.prototype.getState = function () {
  var scope = this.stateStack && this.stateStack[this.stateStack.length - 1];
  return scope;
};
...
Interpreter.prototype.createTask_ = function (isInterval, args) {
  ...
  var node = createNewNode(this.ast);
  var scope, functionRef;

  // setTimeout/setInterval入参为函数时
  if ((exec instanceof Interpreter.Object) && exec.class === 'Function') {
    ...
    let ast;
    // setTimeout/setInterval 入参为字符串时
    try {
...
Interpreter.prototype.nextTask_ = function () {
  ...
  const state = new Interpreter.State(task.node, task.scope);
  // setTimeout/setInterval入参为函数时
  if (task.functionRef) {
    state.doneCallee_ = 2;
    state.funcThis_ = this.globalScope?.object;
...
Interpreter.prototype.initObject = function (globalObject) {
  ...
  wrapper = function Object(value) {
    if (value === undefined || value === null) {
      ...
      if (thisInterpreter.calledWithNew()) {
        ...
        // Called as `Object()`.
        return new Interpreter.Object(thisInterpreter.OBJECT_PROTO);
      }
    }
    if (!(value instanceof Interpreter.Object)) {
      // Wrap the value as an object.
      var box = new Interpreter.Object(thisInterpreter.getPrototype(value))
      box.data = value;
      return box;
    }
  ...
  wrapper = function getOwnPropertyDescriptor(obj, prop) {
    ...
    var pseudoDescriptor = new Interpreter.Object(thisInterpreter.OBJECT_PROTO);
    if (getter || setter) {
...
Interpreter.prototype.initRegExp = function (globalObject) {
  ...
  wrapper = function RegExp(pattern, flags) {
    if (thisInterpreter.calledWithNew()) {
      ...
      var rgx = new Interpreter.Object(thisInterpreter.REGEXP_PROTO);
    }
...
Interpreter.prototype.initError = function (globalObject) {
  ...
  this.ERROR = this.createNativeFunction(function Error(opt_message) {
    if (thisInterpreter.calledWithNew()) {
      ...
      // Called as `Error()`.
      var newError = new Interpreter.Object(thisInterpreter.ERROR.properties['prototype'])
    ...
    var constructor = thisInterpreter.createNativeFunction(
      function (opt_message) {
        if (thisInterpreter.calledWithNew()) {
          ...
          // Called as `XyzError()`.
          var newError = new Interpreter.Object(constructor.properties['prototype'])
          newError.class = 'Error';
        }
        thisInterpreter.populateError(newError, opt_message);
        return newError;
...
Interpreter.prototype.initMath = function (globalObject) {
  var myMath = new Interpreter.Object(this.OBJECT_PROTO);
...
Interpreter.prototype.initJSON = function (globalObject) {
  var wrapper;
  var thisInterpreter = this;
  var myJSON = new Interpreter.Object(this.OBJECT_PROTO);
...
Interpreter.prototype.createArray = function () {
  var array = new Interpreter.Object(this.ARRAY_PROTO);
  // Arrays have length.
...
Interpreter.prototype.createFunctionBase_ = function (argumentLength,
  isConstructor) {
  var func = new Interpreter.Object(this.FUNCTION_PROTO);
  if (isConstructor) {
    var proto = new Interpreter.Object(this.OBJECT_PROTO);
...
Interpreter.prototype.nativeToPseudo = function (nativeObj, opt_cycles) {
  ...
  if (nativeObj instanceof RegExp) {
    var pseudoRegexp = new Interpreter.Object(this.REGEXP_PROTO);
  ...
  if (nativeObj instanceof Date) {
    var pseudoDate = new Interpreter.Object(this.DATE_PROTO);
  ...
  if (nativeObj instanceof Number) {
    pseudoBox = new Interpreter.Object(this.NUMBER.properties['prototype']);
  } else if (nativeObj instanceof String) {
    pseudoBox = new Interpreter.Object(this.STRING.properties['prototype']);
  } else if (nativeObj instanceof Boolean) {
    pseudoBox = new Interpreter.Object(this.BOOLEAN.properties['prototype']);
  }
  if (pseudoBox) {
  ...
  if (Array.isArray(nativeObj)) { // Array.
    ...
    pseudoObj = new Interpreter.Object(this.OBJECT_PROTO);
  }
...
Interpreter.prototype.setProperty = function (obj, name, value, opt_descriptor) {
  ...
  var strict = !this.stateStack || this.getState().scope?.strict;
  if (!(obj instanceof Interpreter.Object)) {
    if (strict) {
...
Interpreter.prototype.createSpecialScope = function (parentScope, opt_object) {
  ...
  var object = opt_object || new Interpreter.Object(null);
  return new Interpreter.Scope(parentScope, parentScope.strict, object);
};

/**
 * 从作用域链中取properties中的属性
 ...
 * @param {!Interpreter.Object|Interpreter.Value} errorConstructor Type of error
 *   (if message is provided) or the value to throw (if no message).
 * @param {string=} opt_message Message being thrown.
 */
Interpreter.prototype.throwException = function (errorConstructor, opt_message) {
  if (!this.globalScope) {
  ...
  if (opt_message === undefined ||
    !(errorConstructor instanceof Interpreter.Object)) {
    var error = errorConstructor; // This is a value to throw, not an error class.
  } else {
    var error = new Interpreter.Object(errorConstructor.properties['prototype']);
 ...
 * @param {string|undefined} label break or return语句有些是带标签.
 */
Interpreter.prototype.unwind = function (type, value, label) {
  ...
  // 遇到未处理的完成类型，则构造一个真实的 JavaScript 错误对象
  var realError;
...
Interpreter.prototype.createSetter_ = function (func, left, value) {
  ...
  var funcThis = Array.isArray(left) ? left[0] : this.globalScope?.object;
  var node = createNewNode(this.ast);
  node.type = 'CallExpression';
  var state = new Interpreter.State(node,
    this.stateStack[this.stateStack.length - 1].scope);
...
Interpreter.prototype.boxThis_ = function (value) {
  if (value === undefined || value === null) {
    ...
    return this.globalScope?.object;
  }
  if (!(value instanceof Interpreter.Object)) {
    // Primitives must be boxed.
    var box = new Interpreter.Object(this.getPrototype(value));
    box.data = value;
    return box;
  }
  return value;
...
Interpreter.prototype['stepBinaryExpression'] = function (stack, state) {
  ...
  switch (node.operator) {
    ...
    case 'in':
      ...
      break;
    case 'instanceof':
      if (!isInherit(rightValue, this.FUNCTION)) {
        this.throwException(this.TYPE_ERROR,
...
Interpreter.prototype['stepCallExpression'] = function (stack, state) {
  ...
  if (!state.doneCallee_) {
    ...
    const nextState = new Interpreter.State(node.callee, state.scope);
    nextState.components = true;
    return nextState;
  }
  ...
  if (!state.doneExec_) {
    ...
    if (funcNode) {
      ...
      if (typeof code !== 'string') {
        ...
        var evalNode = createNewNode(this.ast);
...
Interpreter.prototype['stepIdentifier'] = function (stack, state) {
  ...
  if (this.getterStep_) {
    ...
    return this.createGetter_(func, this.globalScope?.object);
  }
  stack[stack.length - 1].value = value;
};
...
// 访问对象属性或方法的表达式 （obj.a或console.log）
...
Interpreter.prototype['stepObjectExpression'] = function (stack, state) {
  ...
  if (!state.object_) {
    // First execution.
    state.object_ = new Interpreter.Object(this.OBJECT_PROTO);
