...
import { ScopeConstructor, StateConstructor, TaskConstructor } from './constructor';
...
class Interpreter {
  ...
  static Object = ValueFactory.Object;
  ...
  constructor(config) {
    this.initFunc_ = config?.init;
    this.valueFactory = config?.valueFactory || new ValueFactory();
    this.executionEngine = config?.executionEngine || new ExecutionEngine(this);
  ...
  setCode(code, variables?: unknown) {
  ...
  appendCode(code, variables?: unknown) {
 ...
 */
Interpreter.prototype.run = function (code?: unknown, variables?: unknown) {
  if (code) {
    this.setCode(code, variables);
  }
  // 循环执行所有节点直到结束或者暂停
